...
const emptyJob: Omit<Job, "id"> = {
  title: "", description: "", identifier: "", jobId: "", datePosted: "",
  validThrough: "", dateModified: "", hiringOrgName: "", hiringOrgUrl: "",
  hiringOrgLogo: "", hiringOrgSameAs: "", streetAddress: "", addressLocality: "",
  addressRegion: "", postalCode: "", addressCountry: "", employmentType: "",
  workHours: "", jobStartDate: "", jobLocationType: "",
...
function ManageCompanies() {
  ...
  const handleSubmit = () => {
    ...
    if (editId) {
      setCompanies(companies.map((c) => (c.id === editId ? { id: editId, ...form } : c)));
      setEditId(null);
    } else {
      setCompanies([...companies, { id: Date.now().toString(), ...form }]);
    }
      ...
      {subTab === "add" && (
        <div className="adm-form-wrap">
          ...
          <div className="adm-form-actions">
            <button className="adm-btn-primary" onClick={handleSubmit}>{editId ? "Save Changes" : "Add Company"}</button>
...
function ManageJobs() {
  ...
  const handleSubmit = () => {
    ...
    if (editId) {
      setJobs(jobs.map((j) => (j.id === editId ? { id: editId, ...form } : j)));
      setEditId(null);
    } else {
    ...
    <div className="adm-section">
      ...
      {subTab === "add" && (
        <div className="adm-form-wrap">
          ...
          <div className="adm-form-grid">
            {(Object.keys(emptyJob) as (keyof typeof emptyJob)[]).map((key) => (
              <Field key={key} label={toLabel(key)} name={key} value={form[key]} onChange={handleChange} textarea={jobTextareas.includes(key)} />
            ))}
          </div>
          <div className="adm-form-actions">
            <button className="adm-btn-primary" onClick={handleSubmit}>{editId ? "Save Changes" : "Add Job"}</button>
        ...
        <div className="adm-table-wrap">
          ...
          {jobs.length === 0 ? (
            ...
            <div className="adm-table-scroll">
              <table className="adm-table">
                <thead>
                  <tr><th>Title</th><th>Company</th><th>Location</th><th>Type</th><th>Salary</th><th>Posted</th><th>Expires</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  {jobs.map((j) => (
                    <tr key={j.id}>
                      <td>{j.title}</td><td>{j.hiringOrgName}</td>
                      <td>{j.addressLocality}, {j.addressCountry}</td>
                      <td>{j.employmentType}</td>
                      <td>{j.salaryValue} {j.salaryCurrency}</td>
                      <td>{j.datePosted}</td><td>{j.validThrough}</td>
                      <td>
                        <div className="adm-row-actions">
    ...
    <>
      <style>{`
        ...
        .adm-tabs {
          display: flex;
          gap: 0.5rem;
          border-bottom: 2px solid rgba(75,139,190,0.2);
          margin-bottom: 2rem;
        }
        ...
        .adm-subtab {
          ...
          color: #7a8ab8;
          background: #16213e;
          border: 1.5px solid rgba(75,139,190,0.2);
          border-radius: 7px;
        ...
        .adm-table th {
          ...
          text-align: left;
          border-bottom: 1px solid rgba(75,139,190,0.2);
          white-space: nowrap;
        }
      ...
      <div className="adm-page">
        <div className="adm-wrapper">
          ...
          <div className="adm-tabs">
            <button className={`adm-tab ${activeTab === "companies" ? "active" : ""}`} onClick={() => setActiveTab("companies")}>
              Manage Companies
            </button>
            <button className={`adm-tab ${activeTab === "jobs" ? "active" : ""}`} onClick={() => setActiveTab("jobs")}>
              Manage Jobs
            </button>
